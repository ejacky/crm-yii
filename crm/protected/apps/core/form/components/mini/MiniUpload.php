...
class  MiniUpload
{
	...
	public function deleteCategory()
	{
		$file = $this->getFile();
		if (!empty($file)){
			return $this->returnMsg("分类删除失败!,该目录下有其他文件,请确认.", 1);
		}
		$category = $this->getCategory();
		if (!empty($category)){
			return $this->returnMsg("分类删除失败!,该目录下有其他分类,请确认", 1);
		}
		Category::model()->delete('id=:folderId',array(':folderId'=>$this->category));
	...
	{
		return MiniCategory::init()->getSons($this->categoryId) ;
	}
